<?php
 ...
 defined('JPATH_BASE') or die;
   ...
   * @version		1.6
   */
  class plgUserProfilestivole extends JPlugin
  {
	/**
	...
	function onContentPrepareData($context, $data)
	{
		...
		$db->setQuery(
			...
			' AND profile_key LIKE \'profilestivole.%\'' .
			' ORDER BY ordering'
		);
	...
	function onContentPrepareForm($form, $data)
	{
		...
		$lang->load('plg_user_profilestivole', JPATH_ADMINISTRATOR);
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if ($userId && $result && isset($data['profilestivole']) && (count($data['profilestivole'])))
		{
			try
			{
				$db = JFactory::getDbo();
				$db->setQuery('DELETE FROM #__user_profiles WHERE user_id = '.$userId.' AND profile_key LIKE \'profilestivole.%\'');
				if (!$db->query()) {
