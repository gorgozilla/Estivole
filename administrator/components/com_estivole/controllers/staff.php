...
class EstivoleControllerStaff extends JControllerForm
{
	...
	public function execute($task=null)
	{
		...
		if($task=='deleteAvailibility'){
			...
			if($result->success){
				...
				$app->redirect('index.php?option=com_estivole&view=staff&layout=edit&task=staff.edit&controller=staff&member_id='.$result->member_id);
			}else{
			...
			if($result->success){
				$app->enqueueMessage('Profil enregistré avec succès!');
				$app->redirect('index.php?option=com_estivole&view=staffs');
			}else{
				$app->enqueueMessage($result->message, 'error');
				$app->redirect($_SERVER['HTTP_REFERER']);
			}
